...
RUST_BACKTRACE=full cargo run --target-dir /tmp --manifest-path=rust/Cargo.toml rustc -r /tmp/rfcs -d /tmp/public/index/rustc.js
RUST_BACKTRACE=full cargo run --target-dir /tmp --manifest-path=rust/Cargo.toml targets -r /tmp/rfcs -d /tmp/public/index/targets.js
# Copy commands.js
